$(document).ready(function () {
    $("#iptMglt").spinner({
        min: 1,
    }).val(1000000).on("change", function (e) {
        $.each($(".paradas"), function (i, v ) { //i = index, v = value
            //console.log($(v).text());
            // conteudo da celula
            //$(v).prev() = MGLT
            //$(v).prev().prev() = consumables
            var MGLT = $(v).prev().text();
            var consumables = $(v).prev().prev().text();
            
            $(v).text( CalculaParadas(MGLT, consumables) );
    ...
    $(".button").on("click", function (e) {
        self = $(this);
        $.ajax({
            url: self.val() ? self.val() : "https://swapi.co/api/starships/",
            success: function (data, txtStatus, jqXHR) {
                ...
                */
                if (data.next) {
                    $(".proximo").val(data.next).attr("disabled", false);
                }
                else {
                ...
                if (data.previous) {
                    $(".anterior").val(data.previous).attr("disabled", false);
                }
                else {
                ...
                $.each(data.results, function (index, value) {
                    ...
                    */
                    $("#dataGrid > tbody:last-child").append(
                        /*
                            <td>Name</td>
                            <td>Model</td>
                            <td>Consumables</td>
                        ...
                        */
                        "<tr>" +
                            ...
                            "<td class ='consumables'>" + value.consumables + "</td>" +
                            "<td class ='MGLT'>" + value.MGLT + "</td>" +
                            "<td class ='paradas'>" + CalculaParadas(value.MGLT, value.consumables) + "</td>" +
                        "</tr>"
                        );
                });
...
/**
* Função para converter os dados de consumo de uma Starship da api SWAPI em horas.
* @param {String} Consumable SWAPI - starships -> consumables, contem um numero e formato de tempo separados por um espaço(" ")
* @return {int} retorna Valor em horas 
*/
var ConverterParaHora = function (Consumable) {
    var retorno = 0;

        try {
            var vet = Consumable.split(" ");
            var quantidade = vet[0];
            var formato = vet[1];
            var horas = 0;
            switch (formato) //Em todos os calclos foi considerado 1 dia = 24 horas, 1 semana = 7 dias, 1 mês = 30 dias, 1 ano = 365 dias.
            {
                case "year":
                case "years": {
                    horas = 8760; //24*365
...
/**
* Função para calcular número de paradas.
* @param {int} Consumable SWAPI - starships
* @param {int} MGLT SWAPI - starships -> MGLT
* @return {int} retorna numero de paradas para reabastecer. 
*/
var CalculaParadas = function (MGLT, consumables)
{
    /*
     * Considerando a nave abastecida no ponto de partida, por isso se retira uma parada.
     */
    var retorno = "unknown";
    if (consumables != "unknown" && MGLT != "unknown") {
    var hora = ConverterParaHora(consumables);
        retorno = Math.ceil($("#iptMglt").val() / (parseInt(MGLT) * parseInt(hora))) - 1;
    }
    return retorno;
}